...
describe("4 Purchase Punk Test", () => {
  ...
  //   expect(1).to.equal(1);
  // });
  describe("4.1 Testing for proper Vouchers", () => {
    it("4.1.1 Should Revert if Valid vouchers are not passed", async () => {
      const detailsMp = {
  ...
  describe("4.2 Market Place test using $PUNK", async () => {
    it("4.2.1 Should be able to list and Buy NFT if Valid vouchers are passed and have sufficient $PUNK balance", async () => {
      const detailsMp = {
        listingAddress: signer1Address,
        tokenId: tokenIDnum2,
        quantity: 1,
    ...
    it("4.2.2 Should revert if sufficient $PUNK is not present with the buyer", async () => {
      const bal = await token.balanceOf(ownerAddress);
      await token.transferFrom(ownerAddress, signer1Address, bal);
      const detailsMp = {
        listingAddress: signer1Address,
        tokenId: tokenIDnum2,
        quantity: 1,
      ...
      await expect(
        mp.connect(owner)[func.purchasePunk](voucher, purchaseVoucher)
      ).to.be.reverted;
    });
    it("4.2.3 Should be able to purchase the NFT even after the creator is not the owner of the NFT", async () => {
      await mnft
        .connect(signer1)
        .safeTransferFrom(
          signer1Address,
    ...
    it("4.2.4 Should emit event after a successful Purchase", async () => {
      const detailsMp = {
        listingAddress: signer1Address,
        tokenId: tokenIDnum2,
        quantity: 1,
      ...
      expect(
        await mp.connect(owner)[func.purchasePunk](voucher, purchaseVoucher)
      )
        .to.emit(events.purchaseSucessfull)
        .withArgs(anyValue, anyValue, anyValue);
    });
    it("4.2.5 Should Transfer the correct Royality to the creator of the NFT", async () => {
      ...
      const bal = await token.balanceOf(signer1Address);
      let Bbal = new BigNumber(bal);
      const detailsMp = {
        listingAddress: signer2Address,
        tokenId: tokenIDnum2,
        quantity: 1,
      ...
      Bbal = Bbal.plus(ethers.parseEther("10"));
      expect(await token.balanceOf(signer1Address)).to.equal(Bbal);
    });
    it("4.2.6 Should Transfer the correct fees to the Smart contract", async () => {
      await mnft.safeTransferFrom(
        signer1Address,
        signer2Address,
        tokenIDnum2,
      ...
      await mp.connect(owner)[func.purchasePunk](voucher, purchaseVoucher);
      expect(await token.balanceOf(mpAddress)).to.equal(ethers.parseEther("1"));
    });
    it("4.2.7 Should Revert if the caller does not have the RELAYER_ROLE", async () => {
      await mnft.safeTransferFrom(
        signer1Address,
        signer2Address,
        tokenIDnum2,
      ...
      await expect(
        mp.connect(signer1)[func.purchasePunk](voucher, purchaseVoucher)
      )
        .to.be.revertedWithCustomError(mp, errors.acUnauthorized)
        .withArgs(anyValue, anyValue);
    });
    // it("4.2.8 Should be able to list and buy 1155 tokens if valid vouchers are passed and sufficient $PUNK is present", async () => {
    //   const detailsMp = {
    //     listingAddress: signer1Address,
    //     tokenId: tokenIDnum2,
    //     quantity: 2,
    ...
    //   const tx = await mp
    //     .connect(owner)
    //     [func.purchasePunk](voucher, purchaseVoucher, 2);
    //   expect(tx).to.not.be.reverted;
    //   expect(await mnft.balanceOf(ownerAddress, tokenIDnum2)).to.equal(2);
    // });
    // it("4.2.9 Should be able to list and buy 1155 tokens even if the owner of the token is not the creator", async () => {
    //   await mnft.safeTransferFrom(
    //     signer1Address,
    //     signer2Address,
    //     tokenIDnum2,
    ...
    //   expect(await mnft.balanceOf(ownerAddress, tokenIDnum2)).to.equal(1);
    // });
    // it("4.2.10 Should emit event after a successfull Purchase [1155]", async () => {
    //   const detailsMp = {
    //     listingAddress: signer1Address,
    //     tokenId: ethers.toBigInt(tokenId2),
    //     quantity: 2,
    ...
    //   expect(
    //     await mp.connect(owner)[func.purchasePunk](voucher, purchaseVoucher, 2)
    //   )
    //     .to.emit(events.purchaseSucessfull)
    //     .withArgs(anyValue, anyValue, anyValue);
    // });
    // it("4.2.11 Should Transfer the correct Royality to the creator of the 1155", async () => {
    //   await mnft.safeTransferFrom(
    //     signer1Address,
    //     signer2Address,
    //     tokenIDnum2,
    ...
    //   expect(await token.balanceOf(signer1Address)).to.equal(Bbal);
    // });
    // it("4.2.12 Should Transfer the correct fees to the Smart contract [1155]", async () => {
    //   await mnft.safeTransferFrom(
    //     signer1Address,
    //     signer2Address,
    //     tokenIDnum2,
